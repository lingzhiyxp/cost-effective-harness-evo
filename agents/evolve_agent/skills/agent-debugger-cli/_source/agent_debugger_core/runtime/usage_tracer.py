...
This is a verbatim copy of `usage_tracer.py` at the AHE repo root, which is where
edits should be made. `adb` is a console script that resolves imports against its
own installed package, so it cannot import the repo-root module -- and vendoring
a copy keeps agent_debugger_core installable on its own. Keep the two in sync.
"""
